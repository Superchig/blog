...
to a prefab, we have to detect both of these cases.
...
Let's create a method which will check if we're a prefab scene:
...
Now, let's use this `IsPrefab` method to check if we're in the full stage for a
prefab in `Update`:
