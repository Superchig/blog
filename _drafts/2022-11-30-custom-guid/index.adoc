...
standard library], we'll be storing them as strings for a few reasons:
...
public class CustomGUID : MonoBehaviour
{
    ...
    private void Awake()
    {
    	// Assign a new GUID every time a script is attached or when the scene
	// loads:
	guid = Guid.NewGuid();
    }
}
